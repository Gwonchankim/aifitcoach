...
| 8 | 내부 화면에서 세트 행·⋯ 메뉴·RIR 시트·48×48 완료 체크 확인 | 가로 스크롤·겹침·잘림 없이 조작할 수 있다. |
| 9 | 운동 자세에서 한 손으로 반복 조작 | 숫자가 읽히고 ⋯·RIR·완료 체크를 오조작 없이 누를 수 있다. |

각 단계 스크린샷과 세션 URL의 UUID를 남긴다. 3·5·7번 뒤에는 PC에서 DB를 직접 조회해 수행 수,
중복, correlation ID 유출, 대시보드·요약·다음 추천을 화면과 함께 판정한다.

### 2026-08-16 갤럭시 Z 플립6 결과

- 실제 비행기 모드에서 기록·저장이 정상 동작했다.
- 폰 완전 종료와 앱 강제 종료 뒤에도 기록이 복원돼 IndexedDB 커밋 경계를 실기기에서 확인했다.
- 내부 화면의 운동 중 가독성과 한 손 조작감은 양호했다.
- **커버 스크린은 검증 대상에서 제외했다.** 갤럭시 Z 플립6의 커버 화면 실행 앱 목록에 Chrome이 없어
  이 앱의 Chrome PWA를 실행할 수 없다. 따라서 커버 화면과 접기 전환은 현재 지원 환경의 수용 기준이 아니며,
  Chrome의 커버 화면 지원 상태가 바뀌면 별도 실기기 호환성 티켓에서 다시 평가한다.

### M-4′ 실기기 시각 대조 체크리스트 (production-LAN)

이번 확인은 STEP 6 오프라인 내구성 재검증이 아니라 `docs/prototypes/` 정본에 대한 **육안 근접도** 검토다.
2026-08-16 준비 주소는 `https://192.168.0.174:3000`이며 인증서 SAN에 같은 IP가 들어 있다.
고정 390px 목업의 상태줄·외곽 그림자·고정 높이는 제품 UI에 만들지 않은 것이 정상이다.

| # | 경로·확인 | 프로토타입 대비 기대 모습 |
|---|---|---|
| 1 | 네 화면 공통 프레임 | 흰 surface, 좌우 16px gutter, 카드 사이 약 12px, 1px 회색 테두리·3px radius·그림자 없음. 본문은 Pretendard, 숫자만 mono/tabular다. |
| 2 | 하단 4탭 `오늘/기록/프로그램/내 정보` | 화면 아래 50px를 정확히 4등분하고 현재 탭은 파랑 표식+문자로 구분한다. 각 영역은 손가락으로 누르기 충분하고 본문을 가리지 않는다. |
| 3 | 오늘 헤더·오늘 카드 | 좌측 19px `오늘`, 우측 실제 날짜·주차. 강조 카드에는 실제 오늘 상태와 종목/세트 수가 나오며 black/ink 면은 행동, blue는 상태·정보에만 쓴다. |
| 4 | `이번 주 리듬` | 월→일 7열, 열 간격 4px. 완료 ✓·부분 ½·오늘 ●·예정·휴식이 색뿐 아니라 문자로도 구분되고 날짜를 누르면 아래 상세가 열린다. |
| 5 | 홈 e1RM·주간 볼륨 | ready 종목만 큰 e1RM 값과 4주 변화가 보인다. 볼륨은 `34px / track / 54px` 정렬, 9px bar와 실제 범위·이탈 문구를 사용한다. |
| 6 | 기록 탭 0회 종목 `고블릿 스쿼트` | e1RM 값·선·추천이 없고 기록 없음/예정·유사 종목 안내만 보인다. 과거 기록 직접 입력 CTA는 없어야 한다(D-47). |
| 7 | 기록 탭 1회 종목 `벤치프레스` 또는 `랫풀다운` | 날짜 점만 하나 보이고 선·e1RM 값·다음 추천·근거는 DOM에 없어야 한다. 프로토타입의 2회 추천 노출은 따르지 않는다(D-39). |
| 8 | 기록 탭 3회 종목 `체스트 프레스 머신` | 36px e1RM 값, `326×118` 추이선, 실제 최근 세트가 보인다. 3세션 gate를 통과한 뒤에만 추이·추천이 나타난다. |
| 9 | 주간 프로그램 진행 카드 | `12주 중 N주차`, 6px 파랑 progress, 완료 세션 수와 ready e1RM delta가 실제 데이터로 보인다. `무료` chip·일정 바꾸기 버튼은 없다(D-48·D-49). |
| 10 | 주간 프로그램 요일·미래 주차 | 7개 날짜 행은 최소 44px, 완료/부분/진행/예정/휴식 표식이 정렬된다. 펼치면 55px 들여쓰기+dotted divider로 계획 세트가 나오며 미래 주차는 session 행처럼 가장하지 않고 `예정`으로 보인다. |

준비된 `afc` 데이터는 종목별 완료 세션 수가 `고블릿 스쿼트=0`, `벤치프레스/랫풀다운=1`,
`체스트 프레스 머신=3`이다. 8월 10일·12일 기존 lazy 세션에 체스트 프레스 수행 세트 6개를 추가하고
analytics full backfill을 실행했으며, 새 프로그램/미래 세션을 대량 생성하지 않았다.
