...
- 퍼지는 **자식 → 부모** 순서로 실행한다:
  `access_audits` → `auth_sessions` → `performed_sets` → `planned_sets` → `workout_sessions` → `programs` → `calibration_set` → `user_rir_calibration` → `estimated_1rm` → `muscle_weekly_load` → `sync_mutations` → `subscriptions` → `consents` → `users`
...
- 열람·이동권: `GET /me/export`(기계판독). 삭제권과 필수 수집 동의 철회는 `DELETE /me` → 소프트 삭제 후 명시 cutoff의 퍼지 Job으로 처리한다.
