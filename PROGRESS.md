...
3. **D-8~D-10 minor**: 완료 행 가드를 문자열 매칭 → 실제 대비 측정, 360px 뷰포트 회귀 테스트, `ActionBar` sticky 분기 미실행, 카드 크롬 196px/장(같은 안내 3중 표기), before/after 스크린샷 생성 스크립트 미커밋, RIR 시트가 배경 `inert` 미적용(포털 도입 시 해소).
4. **`weekly_completion_rate` 정의 확정**: "계획 준수율"로 못 박았다. "이번 주에 한 운동량"을 원하면 별도 지표·계약이 필요 — 제품 판단.
