...
| M-AUTH+DEPLOY | 단일 소유자 인증·PIPA 최소분·배포 산출물 | 🟡 스테이징 배포·인증 완료, 파일럿 검증 중 (2026-08-23) | — |
...
## 배포 환경 카탈로그 공백 사건 (2026-08-23)

- Vercel + Cloud Run + Neon 스테이징에서 온보딩 프로그램 생성이 모든 입력으로 막힌 근본 원인은 **Neon의 `exercises`가 0건인 배포 절차 공백**이었다. "맨몸 단독 후보 부족"이나 선택 반전은 원인이 아니므로 관련 가설과 D-9 표기는 폐기했고 추천/난이도/카탈로그 규칙은 바꾸지 않았다.
- 명시적 Cloud Run seed Job으로 정본 105종을 upsert했다. 연속 2회 모두 `seeded 105 exercises (table count = 105)`, API 전 페이지 합계·고유 ID **105/105**, dangling substitution **0**, enum 불일치 **0**, `e_pushup` 200을 live 검증했다. 실제 장애 입력(초급·주 3일·바벨/덤벨/머신/케이블/EZ바·통증 없음)으로 프로그램 생성 201과 현재 프로그램 200도 확인했다.
- 마이그레이션은 현재 저장소 기준 **13개 적용, pending 0**이다(요청의 12는 owner auth 마이그레이션 전 기준). DB와 Prisma datamodel의 `CURRENT_DATE` default 및 `estimated_1rm.session_id` relation 메타데이터를 맞춘 뒤 Cloud Run schema-verify Job이 drift 0으로 통과했다.
- **자동 검증이 못 잡는 결함 — 배포 절차 공백:** 로컬은 `db:seed`가 끝난 DB를 쓰고 CI도 자체 DB를 시드해 모두 green이었지만, 새 Neon의 reference data 상태는 별개였다. 교훈은 **"로컬·CI가 green이어도 배포 환경의 데이터 상태는 별개"**다. `DEPLOY_STAGING.md`의 순서를 migration → seed(2회 멱등 확인) → schema/catalog verify → API deploy로 잠근다.
- UX는 빈 카탈로그를 사용자 입력 400으로 위장하지 않고 503으로 분리한다. 400은 장비 스텝에서만 수정 안내를 보이고, 503은 마지막 스텝에서 서버 준비 문제로 안내하며 다른 스텝으로 이동하거나 입력을 바꾸면 이전 오류를 지운다.
- 선택 표현은 반전 수정이 아니라 **일관성·접근성 개선**이다. 목표/프로필 선택·일정·경력·장비·통증·부위 탭·RIR/통증 값·기록 종목·주간 리듬을 공용 세 신호(연한 채움·진한 테두리·굵은 글자)로 통일한다. 하단 네비와 완료 토글은 각각 현재 위치/완료 상태 전용 의미와 아이콘·텍스트 신호가 있어 별도 패턴을 유지한다(ADR-52 재확정).
- 선택 계약 뮤턴트는 `selection-state.ts` 원본 SHA-256 `2B0BE9D665F024F25D82F270738E1539C794E375359ACCDE60FF59C8A54DD9D9`를 ink 솔리드 변이 `9063B1AF99A63EC0965F13076F6BCD9C4ADECA30EF0301D6B3841EA7F6BC22ED`로 바꿨을 때 전용 테스트 **3건 red**, 지정 파일 원복·SHA 일치 뒤 **5/5 green**이었다. 최종 게이트는 contract **31/31**, shared **87**, web **301**, API **289**, E2E 직렬 **76/76**, mobile Chromium **7/7** + WebKit **7/7**, axe **23화면 위반 0**다.
...
> **2026-08-23 재확정:** 아래 표는 당시 구현 이력이다. 현재 계약은 실행 CTA만 ink이고, 칩·탭·라디오 카드를 포함한 선택은 모두 소프트 채움+진한 테두리+굵은 글자다. 선택 반전 결함이 아니라 색 하나에 의존하던 일관성·접근성 문제를 보강한 것이며, 최신 정본은 ADR-52와 위 “배포 환경 카탈로그 공백 사건” 절이다.
