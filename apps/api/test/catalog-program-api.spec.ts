...
import { BASELINE_CATALOG, CURRENT_CATALOG, MACHINE_IDS } from "./support/catalog-extension";
  ...
  it.each([30, 45, 90] as const)(
    ...
    async (minutes) => {
      ...
      for (const baseline of [true, false]) {
        // Exclude precisely the additions through the public API to reproduce baseline106 candidates.
        // No exercise deletion/reseed is used to simulate an old catalog.
        const response = await request(app.getHttpServer())
          .post("/v1/programs/generate")
          .send({ ...input, ...(baseline ? { avoid_exercises: MACHINE_IDS } : {}) })
          .expect(201);
        expectMatchesContract("post", "/programs/generate", 201, response.body);
        const expected = weeklySelection(baseline ? BASELINE_CATALOG : CURRENT_CATALOG, input);
