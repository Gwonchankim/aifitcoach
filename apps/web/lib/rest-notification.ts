 ...
 *
 * ## 권한을 요청하지 않는다 — 그래서 기본값은 "안 뜬다"
 *
 * `Notification.requestPermission()` 을 부르지 않는다. 백그라운드에서 권한 프롬프트를 띄우는 것은
 * 사용자가 요청한 적 없는 방해이고, 제스처 없이 부르면 대부분의 브라우저가 거절한다.
 * **이미 `granted` 일 때만** 쓴다.
 *
 * **한계를 분명히 해 둔다: 앱 안에 권한을 얻는 경로가 하나도 없다.** 따라서 사용자가 브라우저
 * 사이트 설정에서 직접 알림을 허용해 두지 않았다면 이 함수는 언제나 아무것도 하지 않는다.
 * 기기 전제조건은 ① 사이트 알림 권한 `granted` ② 서비스워커 등록 ③ 페이지 프로세스 생존이다.
 * 앱 안에서 권한을 청하는 opt-in UX 는 **별도 티켓 후보**이고 이 파일의 범위가 아니다.
 */
